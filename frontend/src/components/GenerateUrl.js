...
const GenerateUrl = ({ token }) => {
  ...
  return (
    <>
        ...
        {hashedUrl && (
            <div>
                <p>Hashed URL: <a href={`${process.env.REACT_APP_API_URL}tiny/${hashedUrl}`} target="_blank" rel="noopener noreferrer" className="hashed-link">{`${process.env.REACT_APP_API_URL}tiny/${hashedUrl}`}</a></p>
            </div>
        )}
    </>
  );
